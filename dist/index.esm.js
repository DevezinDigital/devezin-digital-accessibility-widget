...
function AccessibilityProvider({ children, config = {}, }) {
    ...
    const features = useMemo(() => ({ ...ALL_FEATURES, ...config.features }), [config.features]);
    const [prefs, setPrefs] = useState(() => readStoredPrefs(storageKey));
    const [systemReducedMotion, setSystemReducedMotion] = useState(() => matchMediaSafe("(prefers-reduced-motion: reduce)"));
    const [systemHighContrast, setSystemHighContrast] = useState(() => matchMediaSafe("(prefers-contrast: more)"));
    ...
    const toggle = useCallback((feature) => setPrefs((prev) => {
    ...
    const setFontSize = useCallback((px) => setPrefs((prev) => ({ ...prev, fontSize: clampFontSize(px) })), []);
    const increaseFontSize = useCallback(() => setPrefs((prev) => ({
        ...prev,
        fontSize: clampFontSize(prev.fontSize + FONT_SIZE_STEP),
    })), []);
    const decreaseFontSize = useCallback(() => setPrefs((prev) => ({
        ...prev,
        fontSize: clampFontSize(prev.fontSize - FONT_SIZE_STEP),
    })), []);
    ...
    return (jsx(AccessibilityContext.Provider, { value: value, children: children }));
}

export { A11Y_VERSION, ALL_FEATURES, AccessibilityProvider, DEFAULT_STORAGE_KEY, FEATURE_META, FEATURE_ORDER, FONT_SIZE_DEFAULT, FONT_SIZE_MAX, FONT_SIZE_MIN, FONT_SIZE_STEP, clampFontSize, enabledFeatures, openAccessibilityMenu, profileHasFeatures, registerAccessibilityOpener, useAccessibility, useAccessibilityPreferences };
